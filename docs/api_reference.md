...
> **Version 1.9.0** — A Shiny for Python bridge to deck.gl (v9.2.10) and MapLibre GL JS (v5.3.1).
...
- [IBM Visual Assets (v1.9)](#ibm-visual-assets-v19)
  - [Sprite Build Pipeline](#sprite-build-pipeline)
- [Security (v1.9)](#security-v19)
  - [sanitizeHtml](#sanitizehtml)
- [Lazy Map Initialization (v1.9)](#lazy-map-initialization-v19)
...
> **Validation (v1.9+):** Both the constructor and `update_tooltip()` use the shared `_validate_tooltip()` helper.  If you pass a dict that is missing the required `"html"` key, a `ValueError` is raised immediately:
> ```python
> widget = MapWidget("mymap", tooltip={"style": {}})  # raises ValueError: tooltip dict must include an 'html' key
> await widget.update_tooltip(session, {"style": {}})  # raises ValueError: tooltip dict must include an 'html' key
> ```
...
## IBM Visual Assets (v1.9)

Expanded sprite library with 10 dorsal (top-down) PNG silhouettes covering Baltic and North Sea species.

### `SPECIES_COLORS` (v1.9 — 10 species)

`SPECIES_COLORS` now contains entries for all 10 species:

| Key | Description | RGBA |
| --- | --- | --- |
| `"Grey seal"` | Slate grey | `[100, 100, 100, 220]` |
| `"Ringed seal"` | Icy blue | `[70, 140, 220, 220]` |
| `"Harbour seal"` | Sandy brown | `[180, 140, 80, 220]` |
| `"Harbour porpoise"` | Dark grey-brown | `[80, 80, 60, 220]` |
| `"Bottlenose dolphin"` | Medium grey | `[140, 140, 120, 220]` |
| `"White-beaked dolphin"` | Light grey | `[200, 200, 200, 220]` |
| `"Atlantic cod"` | Olive green | `[100, 130, 60, 220]` |
| `"Baltic herring"` | Steel blue | `[80, 120, 180, 220]` |
| `"Atlantic salmon"` | Salmon pink | `[200, 100, 80, 220]` |
| `"European smelt"` | Pale green | `[140, 180, 120, 220]` |

### `ICON_ATLAS` (v1.9 — PNG format)

```python
ICON_ATLAS: str
```

Base64-encoded data-URI of a **640×64 PNG** sprite sheet containing ten dorsal (top-down) silhouettes.  The format changed from SVG to PNG in v1.9.0 for reliable WebGL texture upload across all browsers.

### `ICON_MAPPING` (v1.9 — 10 species)
...
| `"Ringed seal"` | 64 | 0 | 64 | 64 | 32 |
| `"Harbour seal"` | 128 | 0 | 64 | 64 | 32 |
| `"Harbour porpoise"` | 192 | 0 | 64 | 64 | 32 |
| `"Bottlenose dolphin"` | 256 | 0 | 64 | 64 | 32 |
| `"White-beaked dolphin"` | 320 | 0 | 64 | 64 | 32 |
| `"Atlantic cod"` | 384 | 0 | 64 | 64 | 32 |
| `"Baltic herring"` | 448 | 0 | 64 | 64 | 32 |
| `"Atlantic salmon"` | 512 | 0 | 64 | 64 | 32 |
| `"European smelt"` | 576 | 0 | 64 | 64 | 32 |

### Sprite Build Pipeline

The atlas is generated by `sprites/build_atlas.py`:

1. Drop source PNG silhouettes into `sprites/src/` (one file per species, named by species).
2. Run `python sprites/build_atlas.py` from the project root.
3. The script packs all PNGs into a single atlas PNG, writes the base64 data URI to `ibm.py`, and prints the updated `ICON_MAPPING` dict.

```bash
python sprites/build_atlas.py
```

Species-specific scaling is configured inside `build_atlas.py` using a `SPECIES_SCALE` dict that maps each species to a relative body-size multiplier, so real proportions are preserved in the sprite sheet.
...
A JavaScript function (defined in `deckgl-init.js`) that sanitizes arbitrary HTML strings before they are assigned to `innerHTML`.

**Behavior:**

- Parses the input string with `DOMParser` into an isolated `text/html` document.
- Strips 12 element types entirely: `script`, `iframe`, `object`, `embed`, `form`, `input`, `button`, `link`, `meta`, `base`, `style`, `svg`.
- Removes all `on*` event-handler attributes (e.g. `onclick`, `onmouseover`, `onerror`).
- Removes `href` and `src` attributes whose values begin with `javascript:`.
- Returns the cleaned `innerHTML` of the sanitized body.

**Applied to:**

| Injection point | Function / code path |
| --- | --- |
| Tooltip `innerHTML` | `showTooltip()` in `deckgl-init.js` |
| Marker popup content | `addMarker()` handler |
| Native MapLibre popup | `add_popup` message handler |

```javascript
// Automatically applied — no user action required
tooltipEl.innerHTML = sanitizeHtml(renderedHtml);
```

---

## Lazy Map Initialization (v1.9)

Maps placed inside Bootstrap tab panels defer their WebGL context creation until the tab containing them is first shown.  This prevents:

- Multiple invisible WebGL contexts competing for GPU resources.
- The deck.gl canvas rendering at zero size (blank map on first reveal).

**How it works:**

1. On page load, `isInVisibleTab()` checks whether the map container is inside a hidden Bootstrap tab by walking the full ancestor chain.
2. If hidden, the map init is deferred and all incoming Shiny messages are queued.
3. When Bootstrap fires a `shown.bs.tab` event for any ancestor tab, `initMap()` is called and `replayDeferredMessages()` processes the queue.
4. Each message handler in the replay queue runs inside its own `try/catch`, so a single bad message does not abort the remaining queue.

No additional Python-side configuration is required; the behavior is automatic for any `MapWidget` whose container is inside a Bootstrap `tabPanel()`.
...
As of v1.9.0, hyphenated field names are also supported: `{iso-a3}`, `{common-name}`.

### XSS Protection (v1.9+)

All rendered tooltip HTML is passed through `sanitizeHtml()` before assignment to `innerHTML`.  Dangerous tags, `on*` event handlers, and `javascript:` URIs are stripped automatically.  See [Security (v1.9)](#security-v19) for full details.
