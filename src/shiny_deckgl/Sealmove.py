...
class IHTRConfig:
    ...
    initial_dist: Optional[np.ndarray] = None  # initial distribution across K clusters

def simulate_IHTR(cfg: IHTRConfig, random_state: Optional[int] = None) -> pd.DataFrame:
    """
    ...
    for t in range(cfg.T):
        records.extend([(t, i, int(states[i])) for i in range(cfg.n_agents)])
        # one-step transitions
        new_states = np.empty_like(states)
        for i, s in enumerate(states):
            new_states[i] = rng.choice(K, p=P[s])
        states = new_states

    df = pd.DataFrame(records, columns=["t", "agent", "cluster"])
    return df
...
class Environment:
    ...
    site_quality: Optional[np.ndarray] = None  # optional per-site preference (utility offset)
...
class SealIBM:
    def __init__(self, env: Environment, params: IBMParams, n_agents: int, rng: Optional[np.random.Generator] = None):
        self.env = env
        self.prm = params
        self.n = n_agents
        ...
        for t in range(T):
            for i, s in enumerate(self.states):
                records.append({
                    ...
                    "energy": s.energy,
                    "at_sea": int(s.at_sea),
                    "haulout_site": -1 if s.current_site is None else s.current_site
                })
            self.step()
